...
/**
 * Sortiranje je ubrzano paralelizacijom. 
 * Bubble-sort algoritam nije moguće paralelzovati u svojoj standardnoj formi.
 * Zbog toga je iskorišćena varijanta bubble-sort algoritma koja je suštinski ista,
 * ali deli prolazak kroz niz na runde, tako da se u jednoj rundi posmatraju samo
 * elementi na parnim, a u narednoj elementi na neparnim indeksima u nizu.
 * Ovaj algoritam sortiranja poznat je pod nazivom odd-even sort.
 * Više o ovom algoritmu možete naći na sledećem linku: https://en.wikipedia.org/wiki/Odd%E2%80%93even_sort
*/
void sort() {
    individual_t tmp, *i1, *i2;

    for(int round = 0; round < NINDIVIDUALS; round++) {
        
        if(round % 2 == 0) {
            #pragma omp parallel for private(i1, i2, tmp)
            for (int i=1; i<NINDIVIDUALS-2; i+=2) {
                i1 = &population[i];
                i2 = &population[i + 1];
            ...
            #pragma omp parallel for private(i1, i2, tmp)
            for (int i=0; i<NINDIVIDUALS-2; i+=2) {
                i1 = &population[i];
                i2 = &population[i + 1];
                if (i1->fitness > i2->fitness) {
                    tmp = *i2;
...
void crossover_select() {
    ...
    #pragma omp parallel for private(i1, i2, new_individual)
    for (int i = 0; i < NINDIVIDUALS; i++) {
