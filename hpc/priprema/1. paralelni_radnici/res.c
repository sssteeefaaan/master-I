...
ucinakRadnika paralelniRadnici() {
    ...
    #pragma omp parallel firstprivate(ukupanRadJednogRadnika)
    {
        ...
        for(int i = 0; i < BROJ_KOFA; i++)
        {
            fasadaPremestiKofu(id, i);
            ukupanRadJednogRadnika += tezineKofa[i];
        }

        printf("T[%d]: %d\n", id, ukupanRadJednogRadnika);

        #pragma omp critical
        {
            if(ukupanRadJednogRadnika > ur.ukupanRad){
                ur.ukupanRad = ukupanRadJednogRadnika;
                ur.idRadnika = id;
            }
        }
        #pragma omp barrier
